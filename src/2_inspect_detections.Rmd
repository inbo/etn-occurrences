...
We need to retrieve the detection data from fishes included in the following animal projects:
...
The detections for these animals are retrieved by selecting on `animal_id_pk` in the `vliz.detections_view` table. These values correspond to the primary identifiers (`id_pk`) of the animals in the `vliz.animals_view` table. 

1. Retrieve all relevant values for `id_pk` in `vliz.animals_view`
...
id_pk <- dbGetQuery(my_con, query)
as_tibble(id_pk)
```

2. Generate vector `animal_id_pk` for selection of the corresponding detection data

```{r}
animal_id_pk <- id_pk %>% pull(id_pk)
```

3. Extract detection data from the specific values for `animal_id_pk` (show top 100 results):
...
as_tibble(detections_top_100)
```

4. Calculate the amount of detections related to the specific `animal_id_pk`
...
as_tibble(number_of_detections)
```
...
as_tibble(detection_field_name)
```
...
as_tibble(detection_field_name_100)
```
...
as_tibble(detection_non_null_latitude)
```
...
as_tibble(detection_non_null_longitude)
```
...
as_tibble(detection_non_null_deployment_lat)
```
...
as_tibble(detection_non_null_deployment_long)
```
...
Conclusion: `latitude` and `deployment_lat` often deviate (sligthly), same for longitude values.
See [issue 62](https://github.com/inbo/etn-occurrences/issues/62)
...
Conclusion: both fields differ and should be checked and cleaned.
Reported in [issue 64](https://github.com/inbo/etn-occurrences/issues/64)
