...
animal_projects <- c(
  "2011_rivierprik",
  "2012_leopoldkanaal",
  "2013_albertkanaal", # Remove 'Sync tag'
  "2014_demer",
  "2015_dijle",
  "2015_phd_verhelst", # Select 'Anguilla anguilla' only
  "homarus",
  "phd_reubens"        # Remove 'Sentinel'
)
```
...
```{r}
animals <- get_animals(my_con, animal_project = animal_projects)

animals <-
  animals %>%
  filter(scientific_name != "Sync tag") %>%
  filter(scientific_name != "Sentinel") %>%
  filter((projectcode == "2015_phd_verhelst" & scientific_name == 'Anguilla anguilla') | (projectcode != "2015_phd_verhelst"))

animal_ids <- animals %>% pull(id_pk)
length(animal_ids)
```
