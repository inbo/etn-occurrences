...
Extract all deployment information for the specific network projects:
...
receiver_metadata <- dbGetQuery(my_con, query)
as_tibble(receiver_metadata)
```

Export `receivers` as .csv:

```{r}
write_csv(receiver_metadata, "../data/raw/receivers.csv", na = "")
```

# Inspect all deployments

```{r}
all_deployments_query <- glue_sql(
    "SELECT *
     FROM vliz.deployments_view AS deployments",
    .con = my_con
  )
all_deployments <- dbGetQuery(my_con, all_deployments_query)
as_tibble(all_deployments)
```
