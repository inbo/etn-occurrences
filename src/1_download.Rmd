...
Show orphaned deployments in the detections (i.e. not associated with any `network_project_code`). These should be fixed (see <https://github.com/inbo/etn-occurrences/issues/71>):

```{r}
orphaned_deployments <-
  detections %>%
  filter(network_project_code == "none" | is.na(network_project_code)) %>%
...
network_project_codes <-
  ...
  unique() %>%
  filter(network_project_code != "none") %>%
  pull()
```
