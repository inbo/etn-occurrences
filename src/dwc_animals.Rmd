...
```{r}
event_capture %<>% mutate(dwc_locality = case_when(
  (capture_location == "Genk" |
     capture_location == "Upstream draining channel Genk" |
     capture_location == "sluis Genk" |
     capture_location == "shipping lock Genk") ~ "Shipping lock Genk",
  (capture_location == "Diepenbeek" |
     capture_location == "Upstream drainage channel Diepenbeek") ~ "Shipping lock Diepenbeek",
  (capture_location == "Hasselt" | 
     capture_location == "Upstream drainage channel Hasselt") ~ "Shipping lock Hasselt",
  TRUE ~ capture_location
  )
)
```
...
```{r}
event_capture %<>% mutate(dwc_decimalLatitude = case_when(
  capture_location == "hatchery Erezée" ~ 50.30605,
  (capture_location == "Genk" |
     capture_location == "Upstream draining channel Genk" |
     capture_location == "sluis Genk" |
     capture_location == "shipping lock Genk") ~ 50.93564,
  (capture_location == "Diepenbeek" |
     capture_location == "Upstream drainage channel Diepenbeek") ~ 50.93803,
  (capture_location == "Hasselt" | 
     capture_location == "Upstream drainage channel Hasselt") ~ 50.93803,
  TRUE ~ round(capture_latitude, digits = 5)
  )
)
```
...
```{r}
event_capture %<>% mutate(dwc_decimalLongitude = case_when(
  capture_location == "hatchery Erezée" ~ 5.54259,
  (capture_location == "Genk" |
     capture_location == "Upstream draining channel Genk" |
     capture_location == "sluis Genk" |
     capture_location == "shipping lock Genk") ~ 5.49701,
  (capture_location == "Diepenbeek" |
     capture_location == "Upstream drainage channel Diepenbeek") ~ 5.43731,
  (capture_location == "Hasselt" | 
     capture_location == "Upstream drainage channel Hasselt") ~ 5.43731,
  TRUE ~ round(capture_longitude, digits = 5)
  )
)
```
...
Inspect `date_of_surgery`:

```{r}
event_surgery %>% 
  group_by(date_of_surgery) %>% 
  count() %>% 
  arrange(desc(n))
```

See [issue 47](https://github.com/inbo/etn-occurrences/issues/47). If the date of surgery is not provided, `date_of_surgery` can be considered to be the same as `catched_date_time`.

```{r}
event_surgery %<>% mutate(
  date_of_surgery = case_when(
    is.na(date_of_surgery) ~ catched_date_time,
    TRUE ~ date_of_surgery
  )
)
```

should follow ISO-8601 format (`yyyy-mm-ddThh:mm:ssZ`). Test:
...
See [issue 47](https://github.com/inbo/etn-occurrences/issues/47)

```{r}
event_surgery %<>% mutate(dwc_locality = case_when(
  capture_location == "hatchery Erezée" ~ capture_location,
    (capture_location == "Genk" |
     capture_location == "Upstream draining channel Genk" |
     capture_location == "sluis Genk" |
     capture_location == "shipping lock Genk") ~ "Shipping lock Genk",
...
```{r}
event_surgery %<>% mutate(dwc_decimalLatitude = case_when(
  capture_location == "hatchery Erezée" ~ 50.30605,
  (capture_location == "Genk" |
     capture_location == "Upstream draining channel Genk" |
     capture_location == "sluis Genk" |
  ...
  (capture_location == "Hasselt" | 
     capture_location == "Upstream drainage channel Hasselt") ~ 50.93803,
  TRUE ~ round(surgery_latitude, digits = 5)
  )
)
```
...
```{r}
event_surgery %<>% mutate(dwc_decimalLongitude = case_when(
  capture_location == "hatchery Erezée" ~ 5.54259,
  (capture_location == "Genk" |
     capture_location == "Upstream draining channel Genk" |
     capture_location == "sluis Genk" |
  ...
  (capture_location == "Hasselt" | 
     capture_location == "Upstream drainage channel Hasselt") ~ 5.43731,
  TRUE ~ round(surgery_longitude, digits = 5)
  )
)
```
...
`utc_release_date_time` should follow ISO-8601 format (`yyyy-mm-ddThh:mm:ssZ`). Test:
...
```{r}
event_release %<>% mutate(dwc_locality = case_when(
  release_location == "Genk" ~ "Upstream shipping lock Genk",
  release_location == "Diepenbeek" ~ "Upstream shipping lock Diepenbeek",
  release_location == "Hasselt" ~ "Upstream shipping lock Hasselt",
  TRUE ~ release_location,
  projectname == "2013 Albertkanaal" & is.na(release_location) ~ capture_location)
)
```
...
```{r}
event_release %<>% mutate(dwc_decimalLatitude = case_when(
  projectname == "2013 Albertkanaal" & is.na(release_location) ~ capture_latitude,
  TRUE ~ round(release_latitude, digits = 5)
  )
)
```
...
```{r}
event_release %<>% mutate(dwc_decimalLongitude = case_when(
  projectname == "2013 Albertkanaal" & is.na(release_location) ~ capture_longitude,  
  TRUE ~ round(release_longitude, digits = 5)
  )
)
```
...
```{r}
write.csv(emof , "../data/processed/emof.csv", na = "", row.names = FALSE)
```
