...
```{r}
write_csv(input_animals, here("data", "raw", "input_animals.csv"), na = "")
write_csv(input_tags, here("data", "raw", "input_tags.csv"), na = "")
```
...
```{r}
event_capture %<>% mutate(dwc_decimalLatitude = sprintf("%.7f", round(capture_latitude, digits = 5)))
```
...
```{r}
event_capture %<>% mutate(dwc_decimalLongitude = sprintf("%.7f", round(capture_longitude, digits = 5)))
```
...
```{r}
event_release %<>% mutate(dwc_decimalLatitude = sprintf("%.7f", round(release_latitude, digits = 5)))
```
...
```{r}
event_release %<>% mutate(dwc_decimalLongitude = sprintf("%.7f", round(release_longitude, digits = 5)))
```
...
```{r}
write_csv(event, here("data", "processed", "event.csv"), na = "")
```
...
```{r}
write.csv(emof, here("data", "processed", "emof.csv"), na = "")
```
