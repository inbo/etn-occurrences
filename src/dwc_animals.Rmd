...
```{r}
event %<>% mutate(dwc_modified = date_modified)
```
...
```{r}
event %<>% mutate(dwc_institutionCode = "")
```
...
```{r}
event %<>% mutate(dwc_datasetName = "Acoustic telemetry data of fish in the Scheldt river basin and the Belgian Part of the North Sea (BPNS)")
```
...
Extended Measurements or Facts related to animal information
...
See [issue46](https://github.com/inbo/etn-occurrences/issues/46). Wait for further mapping.
...
Extended Measurements or Facts related to tag information
...
```{r}
write.csv(emof, "../data/processed/emof.csv")
```
