...
Select all columns starting with prefix `dwc_` and `dwc_meta_`:
...
```{r}
event %<>% select(starts_with("dwc_meta_"), everything())
```

Remove prefix `dwc_` and `dwc_meta_`:

```{r}
colnames(event) <- str_remove(colnames(event), "dwc_")
colnames(event) <- str_remove(colnames(event), "meta_")
```
