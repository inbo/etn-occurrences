...
We only need tag information the tags used in these specific animal project. Filter out the rest:
...
At this point, not all surgery date and/or location information is provided in `input_animals`, see [issue 47](https://github.com/inbo/etn-occurrences/issues/47) and [issue 43] (https://github.com/inbo/etn-occurrences/issues/43). Although all date information will be provided eventually, we will now only include records with surgery data available.
...
`utc_release_date_time` should follow ISO-8601 format (yyyy-mm-ddThh:mm:ssZ; as in: 2017-06-27T12:00:00Z):
...
```{r}
event %<>% mutate(dwc_meta_type = "event" )
```

### modified

```{r}
event %<>% mutate(dwc_meta_modified = date_modified)
```

### language

```{r}
event %<>% mutate(dwc_meta_language = "en")
```

### license

```{r}
event %<>% mutate(dwc_meta_license = "http://creativecommons.org/publicdomain/zero/1.0/")
```
...
```{r}
event %<>% mutate(dwc_meta_rightsHolder = "")
```
...
```{r}
event %<>% mutate(dwc_meta_institutionCode = "")
```

### datasetID

```{r}
event %<>% mutate(dwc_meta_datasetID = "")
```

### datasetName

```{r}
event %<>% mutate(dwc_meta_datasetName = "Acoustic telemetry data of fish in the Scheldt river basin and the Belgian Part of the North Sea (BPNS)")
```

### basisOfRecord

```{r}
event %<>% mutate(dwc_meta_basisOfRecord = "humanObservation")
```
...
There's a one to one relation between the event core and the occurrence extension. For this reason, the 
terms from the occurrence extension can be mapped togeterh with the terms from the event core.
...
Map `dwc_measurementType`, `dwc_measurementValue` and `measurementUnit`:
...
emof_tag_full_id <- input_tags %>% 
    mutate(dwc_measurementType = "Tag full identifier") %>% 
...
```{r}
emof_telemetry_type <- input_tags %>% 
...
```{r}
emof_tag_estimated_life <- input_tags %>% 
...
```{r}
emof_tag_frequence <- input_tags %>% 
    mutate(dwc_measurementType = "Tag emission frequency") %>% 
    mutate(dwc_measurementValue = 
             as.character(
               as.numeric(str_remove(input_tags$frequency, "k")) * 1000)) %>% 
